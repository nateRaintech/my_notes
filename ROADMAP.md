...
tools built into the editor, and completion of the dark theme so that no widget falls back
to the native light styling.

- [x] Dark theme covers every widget the app shows — the reported bug was unreadable tab labels _(done: `resources/dark.qss` styled ~12 widget classes while the app instantiates ~25; anything unnamed fell back to the native Windows style, drawn for a *light* palette. The tab bar arrived with the tabbed editor (#94) and was never added, so it painted dark labels on the dark strip. Added rules for `QTabWidget`/`QTabBar` (normal / selected / hover / disabled, with an accent underline on the active tab), `QDockWidget` + its title bar, `QComboBox` (including the separately-drawn popup view), `QCheckBox`/`QRadioButton` indicators, `QSpinBox`, `QGroupBox`, `QProgressBar`, `QToolButton`, `QDialogButtonBox`, `QMenu::separator`, `QLabel`, `QStackedWidget`, `QSizeGrip`, and focus/disabled states throughout. Icon glyphs can't be fixed by QSS — `image: url(...)` resolves against the process working directory, which a frozen --onefile exe can't rely on — so `ui/icons.py` paints the tab-close and dock title-bar glyphs with QPainter in the theme's colour, applied by `MainWindow.apply_theme` and `TabbedEditor.apply_theme`. The durable part is `tests/test_theme_coverage.py`, which walks the **live** widget tree of the window plus every dialog and fails if any Qt class on screen has no selector in the stylesheet — so the next widget added without styling can't ship unreadable. — #98)_
- [x] A suite of text and data tools, applied to the selection in the editor _(done: `core/tools/` — a registry of 63 pure `str -> str` transformations across 10 categories (JSON / XML / SQL / Markdown / Encode-Decode / Hash / Case / Lines / Convert / Insert), Qt-free so the whole suite unit-tests in milliseconds. All three UI surfaces — the Tools menu, the editor's right-click submenu, and the Ctrl+Shift+T fuzzy palette — are **generated from `ALL_TOOLS`**, so adding a tool is one function plus one registry line and it appears in every surface at once; tests assert menu and palette coverage against the registry so they cannot drift. `ui/tool_runner.py` owns the behaviour that makes it feel native: operates on the selection or, with none, the whole note; replaces through one `QTextCursor` inside `beginEditBlock`/`endEditBlock` so a single Ctrl+Z reverts the whole operation; leaves the result selected so tools chain; and on `ToolError` leaves the document byte-identical while reporting the failure's line and column. Three tool modes: `transform` (replace), `generate` (insert at the caret — never replacing the note), and `inspect` (hashes, validation, timestamp conversion: never touches the document, result goes to the status bar and clipboard). `pyyaml` and `sqlparse` are imported lazily inside the tools that need them, so a missing package disables one tool family instead of breaking `core.tools`; both are named in `my_notes.spec` `hiddenimports` because PyInstaller cannot see a runtime `__import__`. 168 new tests. — #99)_
